  ...
  const fetchNews = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("news")
        .select("id, title, imageurl, postdate, updatedate, status")
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center text-gray-400 text-sm">
                      ...
                      <span>Posted: {formatDate(item.postdate)}</span>
                    </div>
                    {item.updatedate !== item.postdate && (
                      <div className="flex items-center text-gray-400 text-sm">
                        <Clock className="w-4 h-4 mr-2" />
                        <span>Updated: {formatDate(item.updatedate)}</span>
                      </div>
                    )}
                  </div>
